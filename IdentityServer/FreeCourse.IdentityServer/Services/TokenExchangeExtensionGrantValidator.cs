...
namespace FreeCourse.IdentityServer.Services
{
    public class TokenExchangeExtensionGrantValidator : IExtensionGrantValidator
    {
        ...
        public async Task ValidateAsync(ExtensionGrantValidationContext context)
        {
            ...
            var subjectClaim = tokenValidateResult.Claims.FirstOrDefault(k => k.Type.Equals("sub"));
            if (subjectClaim == null)
            {
                context.Result = new GrantValidationResult(IdentityServer4.Models.TokenRequestErrors.InvalidClient, "token must contain sub value");
                return;
            }
